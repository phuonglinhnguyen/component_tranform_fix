...
const CommonList: React.FC<IDefautProps, IDefautState> = (props) => {
	...
	const commonData = filter(common, (commonItem) => {
		const key = Object.keys(commonItem)[1];
					...
					{commonData.map((commonItem) => {
						...
						return (
							<ListItem
								...
								onClick={() => {
									setSelectedCommonNameConstant(key)
							...
							>
								...
								<ListItemSecondaryAction>
									...
									<Dialog open={isOpen} onClose={() => setIsOpen(false)}>
										<DialogContent>
											You sure delete common <span style={{ color: 'red', fontWeight: 'bolder' }}>{keyDel}</span> ?{' '}
										</DialogContent>
										<DialogActions>
